...
with st.expander("📊 **How REACH Works: Flow Diagrams**", expanded=False):
    ...
    st.image(os.path.join(DIAGRAMS_DIR, "reach-diag-1.png"), caption="Step 1: Data Upload Flow", use_container_width=False, width=400)
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="image-container">', unsafe_allow_html=True)
    st.image(os.path.join(DIAGRAMS_DIR, "reach-diag-2.png"), caption="Step 2: LLM Analysis & Insights Flow", use_container_width=False, width=400)
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="image-container">', unsafe_allow_html=True)
    st.image(os.path.join(DIAGRAMS_DIR, "reach-diag-3.png"), caption="Step 3: AI Agent Flow", use_container_width=False, width=400)
    st.markdown('</div>', unsafe_allow_html=True)